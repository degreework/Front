...
$( document ).ready(function() {
   ...
   //main_chat_users_num( 2, 0 );
    ...
    $( " #slide-bar" ).tipsy({ fade: true, gravity: $.fn.tipsy.autoBounds( 150, "s" ) });
    $( " #slide-bar, #chat-title-button" ).button();

    //Click for expand and collapse the bar 
...
/*
    ...
    $( document ).on("click", ".user", function() {
      ...
      // aqui enviar el emmit para el historial 
      user = JSON.parse(localStorage.getItem('user'))

      // para saber cuales chats estan activos en el momento 
      ChatSocked.prototype.add_current_chat(user.id, iduser, $(this).text())
      //ChatSocked.prototype.sentMessage('chat_history', iduser+','+user.id) 
    ...
    $( document ).on("click", ".user-status", function() {
      ...
      if ( id == "user-status-online" && now != "http://127.0.0.1:8000/static/images/button_online.png" ) {
        status = "online";
        label  = i18n.connected;
      }

      else if ( id == "user-status-busy" && now != "http://127.0.0.1:8000/static/images/busy.png" ) {
        status = "busy";
        label  = i18n.busy;
      }

      else if ( id == "user-status-offline" && now != "http://127.0.0.1:8000/static/images/button_offline.png" ) {
        status = "offline";
        label  = i18n.offline;
      }
          ...
          if ( $( this ).data( "init" ) != 1 ) {
            ...
            main.parent().find( ".ui-dialog-titlebar-close" ).click(function() {
              ...
              for (var i = 0; i < currentChats.chats.length; i++) {
                ...
                //console.log(chat.to +' === '+ iduser)  

                if(chat.to === iduser ){
                  //console.log('entro al if')
                  currentChats.chats.splice(i,1);
                }
              };
            ...
            main.find( "textarea" ).first().keyup(function( e ) {
              ...
              if ( (e.which == 13) && !event.shiftKey ) {
                ...
                if (msg !== "") {
                  ...
                  ChatSocked.prototype.sentMessage('message', { 'to': id, 'msg': msg, 'ids': id+','+user.id }) /*, function (data) {
                  ...
                  append_msg_me(msg, main, new Date().toISOString());
                }
              }
              return false;
            });
   ...
   // function user_status ( action, data ) {
      ...
      if (he.children().last().attr('id') == 'he') {
        ...
        he.append("\
          <div class='direct-chat-msg' id='he'>\
            ...
            <!--<img class='direct-chat-img' src='http://127.0.0.1:8080" + avatar[0] + "' alt='message user image' />-->\
            <div class='direct-chat-text'>\
              <div>" + msg + "</div>\
            </div>\
          </div>");
    ...
    function main_chat_status ( text , status ) {
      ...
      if ( status == "online" )
        $( "#rerun-img" ).attr( "src", "http://127.0.0.1:8000/static/images/button_online.png" );
      //else if ( status == "busy" )
        //$( "#rerun-img" ).attr( "src", "http://127.0.0.1:8000/static/images/busy.png" );
      //else if ( status == "offline" )
        //$( "#rerun-img" ).attr( "src", "http://127.0.0.1:8000/static/images/button_offline.png" );
    ...
    function main_append_dialog ( id, user ) {
      ...
      if ( $( "#Dialog" + id ).length == 0 ) {
        $( "body" ).append( "\
          <div id='Dialog" + id + "' title='' user='" + user + "' hidden>\
                                                                              <!-- MUST CHANGE -->\
            <div class='box box-info direct-chat direct-chat-info ui-widget-content no-content-border'>\
              ...
              <div class='small-box-footer'>\
                <textarea id='textarea_msg' class='form-control textarea-msg' style='resize:none' name='message' placeholder='"+ i18n.type_message +"'></textarea>\
              </div>\
            </div>\
          </div>");
      }
